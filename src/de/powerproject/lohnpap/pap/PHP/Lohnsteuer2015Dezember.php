...
class Lohnsteuer2015Dezember implements LohnsteuerInterface {
	...
	protected $zveEkSt = new BigDecimal(0);
	protected $zveGemeinsam = new BigDecimal(0);
	...
	protected $ZRE4VPM = new BigDecimal(0);
	/** Neu 2015 */
	...
	protected static final $ZAHL0 = BigDecimal::$ZERO;
...
/** Neu 2015 Nur Änderung an der Bezeichnung */
	...
	protected function UPLSTLZZ() {
		...
		$JW = $LSTJAHR->multiply(self::$ZAHL100);/** Anfang Neu 2015 */
		if($SCHLEIFZ == 1) {
			$JWLSTA = $JW;
		} else {
			$JWLSTN = $JW;
	...
	protected function MSOLZ() {
		...
		if($JBMG->compareTo ($SOLZFREI) == 1) {
			...
			$JW= $SOLZJ->multiply (self::$ZAHL100).setScale (0, BigDecimal::$ROUND_DOWN);/** Anfang Neu 2015 */
			if($SCHLEIFZ == 1) {
				$JWSOLZA = $JW;
			} else {
				$JWSOLZN = $JW;
		...
		if($R > 0) {
			$JW= $JBMG->multiply (self::$ZAHL100);/** Anfang Neu 2015 */
			if($SCHLEIFZ == 1) {
				$JWBKA = $JW;
			} else {
				$JWBKN = $JW;
