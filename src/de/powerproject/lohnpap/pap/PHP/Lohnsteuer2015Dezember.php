...
/**
 * Klasse Lohnsteuer2015Dezember
 * 
 * @author Christian Delfs
 */

class Lohnsteuer2015Dezember implements LohnsteuerInterface {
	...
	public function main() {
		...
		$this->KENNVMT= 0;
		$this->MRE4();
		$this->MRE4ABZ();		/** Anfang Neu 2015 */

		$this->ZRE4VPM= $this->ZRE4VP;
		$this->SCHLEIFZ = 1;
		$this->MBERECH();
		$this->SCHLEIFZ = 2;
		$this->W1STKL5 = BigDecimal::valueOf(9873);
		$this->ZRE4VP=$this->ZRE4VPM;
		$this->MBERECH();
		$this->MLST1215();		/** Ende Neu 2015 */
...
<br>
	...
	protected function MPARA() {
		...
		if($this->KRV < 2) /** &lt; = < */{
			if($this->KRV == 0) {
				$this->BBGRV = new BigDecimal(72600);
			} else {
				$this->BBGRV = new BigDecimal(62400);
			}
			$this->RVSATZAN = BigDecimal::valueOf(0.0935);
			$this->TBSVORV = BigDecimal::valueOf(0.60);
		} else {/** Nichts zu tun */
		}
		$this->BBGKVPV = new BigDecimal(49500);
		...
		$this->W1STKL5 = new BigDecimal(9763);
		$this->W2STKL5 = new BigDecimal(26441);
		$this->W3STKL5 = new BigDecimal(200584);/** Ende Neu 2015 */
	}
...
/** Neu 2015 Nur Änderung an der Bezeichnung */
	...
	protected function MZTABFBN() {
		...
		if($this->STKL == 1) {
			$this->SAP= BigDecimal::valueOf (36);
			$this->KFB= ($this->ZKF->multiply (BigDecimal::valueOf (7152)))->setScale (0, BigDecimal::$ROUND_DOWN);
		} else {
			if($this->STKL == 2) {
				...
				if($this->STKL == 3) {
					...
					if($this->STKL == 4) {
						$this->SAP= BigDecimal::valueOf (36);
						$this->KFB= ($this->ZKF->multiply (BigDecimal::valueOf (3576)))->setScale (0, BigDecimal::$ROUND_DOWN);
					} else {
						if($this->STKL == 5) {
							$this->SAP= BigDecimal::valueOf (36);
							$this->KFB= BigDecimal::$ZERO;
	...
	protected function UPLSTLZZ() {
		...
		$this->JW = $this->LSTJAHR->multiply(self::ZAHL100());/** Anfang Neu 2015 */
		if($this->SCHLEIFZ == 1) {
			$this->JWLSTA = $this->JW;
		} else {
			$this->JWLSTN = $this->JW;
	...
	protected function UPMLST() {
		...
		if($this->STKL < 5) {/** Anfang Neu 2015 */
			if($this->SCHLEIFZ == 1) {
				$this->UPTAB14();
			} else {
				$this->UPTAB15();
	...
	protected function UPEVP() {
		...
		if($this->KRV > 1) /** &lt; = < &gt; = > */{
			...
			$this->VSP1 = ($this->VSP1->multiply($this->RVSATZAN))->setScale(2,BigDecimal::$ROUND_DOWN);/** Neu 2015 Ende */
		}
	...
	protected function UP5_6() {
		...
		$this->X= ($this->ZX->multiply (BigDecimal::valueOf (1.25)))->setScale (2, BigDecimal::$ROUND_DOWN);/** Anfang Neu 2015 */
		if($this->SCHLEIFZ == 1) {
			$this->UPTAB14();
		} else {
			$this->UPTAB15();
		...
		$this->ST1= $this->ST;
		$this->X= ($this->ZX->multiply (BigDecimal::valueOf (0.75)))->setScale (2, BigDecimal::$ROUND_DOWN);/** Anfang Neu 2015 */
		if($this->SCHLEIFZ == 1) {
			$this->UPTAB14();
		} else {
			$this->UPTAB15();
	...
	protected function MSOLZ() {
		...
		if($this->JBMG->compareTo ($this->SOLZFREI) == 1) {
			...
			$this->JW= $this->SOLZJ->multiply (self::ZAHL100())->setScale (0, BigDecimal::$ROUND_DOWN);/** Anfang Neu 2015 */
			if($this->SCHLEIFZ == 1) {
		...
		if($this->R > 0) {
			$this->JW= $this->JBMG->multiply (self::ZAHL100());/** Anfang Neu 2015 */
			if($this->SCHLEIFZ == 1) {
				$this->JWBKA = $this->JW;
			} else {
				$this->JWBKN = $this->JW;
	...
	protected function MLST1215() {

		if($this->LZZ > 1) {
			$this->JW = ($this->JWLSTN->subtract(BigDecimal::valueOf(11)->multiply(($this->JWLSTA->subtract($this->JWLSTN)))))->setScale(0, BigDecimal::$ROUND_DOWN);
			if($this->JW->compareTo(BigDecimal::$ZERO) == -1) /** JW < 0 */{
				$this->ANTEIL1 = BigDecimal::$ZERO;
			} else {
				$this->UPANTEIL();
			}
			$this->LSTLZZ = $this->ANTEIL1;
			$this->JW = ($this->JWSOLZN->subtract(BigDecimal::valueOf(11)->multiply($this->JWSOLZA->subtract($this->JWSOLZN))))->setScale(0, BigDecimal::$ROUND_DOWN);
			if($this->JW->compareTo(BigDecimal::$ZERO) == -1) /** JW < 0 */{
				$this->ANTEIL1 = BigDecimal::$ZERO;
			} else {
				$this->UPANTEIL();
			...
			$this->SOLZLZZ = $this->ANTEIL1;
			$this->JW = ($this->JWBKN->subtract(BigDecimal::valueOf(11)->multiply($this->JWBKA->subtract($this->JWBKN))))->setScale(0, BigDecimal::$ROUND_DOWN);
			if($this->JW->compareTo(BigDecimal::$ZERO) == -1) /** JW < 0 */{
				$this->ANTEIL1 = BigDecimal::$ZERO;
			} else {
				$this->UPANTEIL();
	...
	protected function UPTAB14() {
		...
		if($this->X->compareTo (BigDecimal::valueOf (8355)) == -1) {
			$this->ST= BigDecimal::$ZERO;
		} else {
			if($this->X->compareTo (BigDecimal::valueOf (13470)) == -1) {
				$this->Y= ($this->X->subtract (BigDecimal::valueOf (8354)))->divide (BigDecimal::valueOf (10000), 6, BigDecimal::$ROUND_DOWN);
				$this->RW= $this->Y->multiply (BigDecimal::valueOf (974.58));
				...
				if($this->X->compareTo (BigDecimal::valueOf (52882)) == -1) {
					...
					$this->ST= ($this->RW->add (BigDecimal::valueOf (971)))->setScale (0, BigDecimal::$ROUND_DOWN);
				} else {
					if($this->X->compareTo (BigDecimal::valueOf (250731)) == -1) {
	...
	protected function UPTAB15() {

		if($this->X->compareTo (BigDecimal::valueOf (8473)) == -1) {
			$this->ST= BigDecimal::$ZERO;
		} else {
			if($this->X->compareTo (BigDecimal::valueOf (13470)) == -1) {
				$this->Y= ($this->X->subtract (BigDecimal::valueOf (8472)))->divide (BigDecimal::valueOf (10000), 6, BigDecimal::$ROUND_DOWN);
				$this->RW= $this->Y->multiply (BigDecimal::valueOf (997.60));
				$this->RW= $this->RW->add (BigDecimal::valueOf (1400));
				$this->ST= ($this->RW->multiply ($this->Y))->setScale (0, BigDecimal::$ROUND_DOWN);
			} else {
				if($this->X->compareTo (BigDecimal::valueOf (52882)) == -1) {
					...
					$this->RW= $this->RW->add (BigDecimal::valueOf (2397));
					$this->RW= $this->RW->multiply ($this->Y);
					$this->ST= ($this->RW->add (BigDecimal::valueOf (948.68)))->setScale (0, BigDecimal::$ROUND_DOWN);
				} else {
					if($this->X->compareTo (BigDecimal::valueOf (250731)) == -1) {
						$this->ST= (($this->X->multiply (BigDecimal::valueOf (0.42)))->subtract (BigDecimal::valueOf (8261.29)))->setScale (0, BigDecimal::$ROUND_DOWN);
					} else {
						$this->ST= (($this->X->multiply (BigDecimal::valueOf (0.45)))->subtract (BigDecimal::valueOf (15783.19)))->setScale (0, BigDecimal::$ROUND_DOWN);
					}
				}
			}
		}
