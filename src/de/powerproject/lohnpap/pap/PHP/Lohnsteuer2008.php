...
class Lohnsteuer2008 implements LohnsteuerInterface {
	...
	/** Sicherungsfeld für den Zuschlag zum Versorgungsfreibetrag in € für<br>
             ...
             Tätigkeit */
	protected $FVBZOSO = new BigDecimal(0);
	...
	protected $ZRE4OSO = new BigDecimal(0);
	...
	protected $ZRE4VMT = new BigDecimal(0);
	...
	protected $ZTABFBOSO = new BigDecimal(0);
	...
	protected function MRE4() {
		...
		if($ZVBEZJ->compareTo (BigDecimal::$ZERO) == 0) {
			...
			if((self::$TAB3[3]).compareTo ($HFVBZSO) == 1) {
	...
	protected function MRE4ALTE() {
		...
		if($ALTER1 == 0) {
			...
			$ALTE= ($BMG->multiply (self::$TAB4[$K])).setScale (2, BigDecimal::$ROUND_UP);
			$HBALTE= self::$TAB5[$K];
	...
	protected function MLSTJAHR() {
		...
		if($KENNVMT == 0) {
			...
			if($KENNVMT == 1) {
				...
				$ZVE= ((($ZRE4->subtract ($ZTABFB)).divide (self::$ZAHL5)).subtract ($VSP)).setScale (2, BigDecimal::$ROUND_DOWN);
			}
		}
...
/** Achtung: Er wird davon ausgegangen, dass<br>
	...
	protected function MSONST() {
		...
		if($SONSTB->compareTo (BigDecimal::$ZERO) == 0) {
			...
			MRE4SONST();
			MLSTJAHR();
			$LSTSO= $ST->multiply (self::$ZAHL100);
	...
	protected function MVMT() {
		...
		if(($VMT->add ($VKAPA)).compareTo (BigDecimal::$ZERO) == 1) {
			...
			if(($JRE4->add ($SONSTB).subtract ($JFREIB).add ($JHINZU)).compareTo (BigDecimal::$ZERO) == -1) {
				...
				$ZRE4VMT= ((($VMT->divide (self::$ZAHL100)).add ($VKAPA->divide (self::$ZAHL100)).subtract ($ZTABFB).add ($ZTABFBOSO)).divide (self::$ZAHL5)).setScale (2, BigDecimal::$ROUND_DOWN);
				MLSTJAHR();
				$LST2= $ST->multiply (self::$ZAHL100);
