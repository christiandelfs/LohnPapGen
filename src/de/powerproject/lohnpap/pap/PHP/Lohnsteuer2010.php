...
class Lohnsteuer2010 implements LohnsteuerInterface {
	...
	$this->KVSATZAN=new BigDecimal(0)->setScale(5);
	$this->KENNVMT=0;
	$this->LST3=new BigDecimal(0);
	$this->LST2=new BigDecimal(0);
	$this->LST1=new BigDecimal(0);
	$this->KVSATZAG=new BigDecimal(0)->setScale(5);
	$this->VJAHR=0;
	...
	$this->PVSATZAG=new BigDecimal(0)->setScale(5);
	$this->STV=new BigDecimal(0);
	$this->PVSATZAN=new BigDecimal(0)->setScale(5);
	$this->PVZ=0;
	...
	protected function UPEVP() {
		...
		if($this->KRV > 1) {
			...
			if($this->KRV == 0) {
				...
				if($this->ZRE4VP->compareTo (BigDecimal::valueOf (55800)) == 1) {
					$this->ZRE4VP= BigDecimal::valueOf (55800);
				}
			}
			$this->VSP1= $this->ZRE4VP->multiply (BigDecimal::valueOf (0.4))->setScale (2, BigDecimal::$ROUND_DOWN);
	...
	protected function MVSP() {
		...
		if($this->ZRE4VP->compareTo( BigDecimal::valueOf(45000) ) == 1) {
			$this->ZRE4VP = BigDecimal::valueOf(45000L);
		}
		if($this->PKV > 0) {
			if($this->STKL == 6) {
				...
				if($this->PKV == 2) {
					$this->KVSATZAG = BigDecimal::valueOf(0.067)->setScale(5);
					if($this->PVS == 1) {
						$this->PVSATZAG = BigDecimal::valueOf(0.00475D)->setScale(5);
					} else {
						$this->PVSATZAG = BigDecimal::valueOf(0.00975D)->setScale(5);
					}
					$this->VSP = $this->VSP->subtract($this->ZRE4VP->multiply($this->KVSATZAG->add($this->PVSATZAG)))->setScale(2, BigDecimal::$ROUND_DOWN);
				}
			}
		} else {
			$this->KVSATZAN = BigDecimal::valueOf(0.076D)->setScale(5);
			if($this->PVS == 1) {
				$this->PVSATZAN = BigDecimal::valueOf(0.01475D)->setScale(5);
			} else {
				$this->PVSATZAN = BigDecimal::valueOf(0.00975D)->setScale(5);
			}
			if($this->PVZ == 1) {
				$this->PVSATZAN = $this->PVSATZAN->add(BigDecimal::valueOf(0.0025D));
			}
