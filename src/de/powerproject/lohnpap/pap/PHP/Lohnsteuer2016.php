...
class Lohnsteuer2016 implements LohnsteuerInterface {
	...
	protected $KRV = 0;
	protected $KVZ = new BigDecimal(0);
	protected $LZZ = 0;
	...
	protected $ZMVB = 0;
	protected $JRE4ENT = BigDecimal.ZERO;
	protected $SONSTENT = BigDecimal.ZERO;
	...
	protected $VKVLZZ = new BigDecimal(0);
	protected $VKVSONST = new BigDecimal(0);

	/* AUSGABEPARAMETER DBA*/

	protected $VFRB = new BigDecimal(0);
	protected $VFRBS1 = new BigDecimal(0);
	protected $VFRBS2 = new BigDecimal(0);
	protected $WVFRB = new BigDecimal(0);
	protected $WVFRBO = new BigDecimal(0);
	protected $WVFRBM = new BigDecimal(0);
	...
	protected $GFB = new BigDecimal(0);
	...
	protected $KVSATZAG = new BigDecimal(0);

	/** Beitragssatz des Arbeitnehmers zur Krankenversicherung */
	protected $KVSATZAN = new BigDecimal(0);
	...
	protected $PVSATZAG = new BigDecimal(0);

	/** Beitragssatz des Arbeitnehmers zur Pflegeversicherung */
	protected $PVSATZAN = new BigDecimal(0);

	/** Beitragssatz des Arbeitnehmers in der allgemeinen gesetzlichen Rentenversicherung (4 Dezimalstellen) */
	protected $RVSATZAN = new BigDecimal(0);
	...
	protected $TBSVORV = new BigDecimal(0);
	...
	protected $VSP3 = new BigDecimal(0);

	/** Erster Grenzwert in Steuerklasse V/VI in Euro */
	protected $W1STKL5 = new BigDecimal(0);

	/** Zweiter Grenzwert in Steuerklasse V/VI in Euro */
	protected $W2STKL5 = new BigDecimal(0);

	/** Dritter Grenzwert in Steuerklasse V/VI in Euro */
	protected $W3STKL5 = new BigDecimal(0);
	...
	protected $VKV = new BigDecimal(0);
	...
	protected static final $ZAHL1000 = new BigDecimal(1000);
	protected static final $ZAHL10000 = new BigDecimal(10000);
	...
	protected function MPARA() {
		...
		$KVSATZAN = ($KVZ->divide(self::$ZAHL100)).add(BigDecimal::valueOf(0.07));
	...
	protected function MBERECH() {
		...
		MZTABFB();/** Neu 2016 */
		$VFRB = (($ANP->add($FVB->add($FVBZ))).multiply(self::$ZAHL100)).setScale(0, BigDecimal::$ROUND_DOWN);/** Neu 2016 */
		MLSTJAHR();/** Anfang Neu 2016 */
		$WVFRB = (($ZVE->subtract($GFB)).multiply(self::$ZAHL100)).setScale(0, BigDecimal::$ROUND_DOWN);
	...
	protected function MZTABFB() {
		...
		if($STKL < 6) {
			if($ZRE4->compareTo($ZVBEZ) == 1) {
				if($ZRE4->subtract($ZVBEZ).compareTo(self::$ZAHL1000) == -1) {
					$ANP = $ANP->add($ZRE4).subtract($ZVBEZ).setScale(0,BigDecimal::$ROUND_UP);
				} else {
					$ANP = $ANP->add(self::$ZAHL1000);
				}
			}
		}
		$KZTAB= 1;
	...
	protected function UPVKV() {
		...
		if($PKV > 0) {
			if($VSP2->compareTo($VSP3) == 1) {
				$VKV = $VSP2->multiply(self::$ZAHL100);
			} else {
				$VKV = $VSP3->multiply(self::$ZAHL100);
			}
		} else {
			$VKV = BigDecimal::$ZERO;
		}
	...
	protected function MVSP() {
		...
		if($PKV > 0) {
			if($STKL == 6) {
				...
				$VSP3 = $PKPV->multiply(self::$ZAHL12).divide(self::$ZAHL100);
				if($PKV == 2) {
	...
	protected function MSONST() {
		...
		if($SONSTB->compareTo (BigDecimal::$ZERO) == 0) {
			...
			MRE4SONST();
			MLSTJAHR();/** Anfang Neu 2016 */
			$WVFRBM = ($ZVE->subtract($GFB)).multiply(self::$ZAHL100).setScale(2,BigDecimal::$ROUND_DOWN);
	...
	protected function MVMT() {
		...
		if(($VMT->add ($VKAPA)).compareTo (BigDecimal::$ZERO) == 1) {
			...
			MRE4ABZ();
			$ZRE4VP = $ZRE4VP->subtract($JRE4ENT->divide(self::$ZAHL100)).subtract($SONSTENT->divide(self::$ZAHL100));
			$KENNVMT= 1;
			MLSTJAHR();
	...
	protected function MOSONST() {
		...
		MRE4();
		MRE4ABZ();
		$ZRE4VP = $ZRE4VP->subtract($JRE4ENT->divide(self::$ZAHL100));
		MZTABFB();/** Neu 2016 */
		$VFRBS1 = (($ANP->add($FVB->add($FVBZ))).multiply(self::$ZAHL100)).setScale(2,BigDecimal::$ROUND_DOWN);
		MLSTJAHR();/** Anfang Neu 2016 */
		$WVFRBO = (($ZVE->subtract($GFB)).multiply(self::$ZAHL100)).setScale(2, BigDecimal::$ROUND_DOWN);
	...
	protected function MRE4SONST() {
		...
		$FVBZ= $FVBZSO;
		MZTABFB();/** Neu 2016 */
		$VFRBS2 = (((($ANP->add($FVB).add($FVBZ))).multiply(self::$ZAHL100))).subtract($VFRBS1);
	}
	...
	protected function UPTAB16() {
		...
		if($X->compareTo($GFB->add(self::$ZAHL1)) == -1) {
			$ST= BigDecimal::$ZERO;
		} else {
			if($X->compareTo (BigDecimal::valueOf (13670)) == -1) {
				$Y = ($X->subtract($GFB)).divide(self::$ZAHL10000, 6,BigDecimal::$ROUND_DOWN);
				...
				if($X->compareTo (BigDecimal::valueOf (53666)) == -1) {
					$Y= ($X->subtract (BigDecimal::valueOf (13669))).divide (self::$ZAHL10000, 6, BigDecimal::$ROUND_DOWN);
