...
/**
 * Klasse Lohnsteuer2016
 * 
 * @author Christian Delfs
 */

class Lohnsteuer2016 implements LohnsteuerInterface {
	...
	public function setKVZ($arg0) { $this->KVZ = $arg0; }
	...
	public function getWVFRBM() { return $this->WVFRBM; }

	
	public function getWVFRB() { return $this->WVFRB; }

	
	public function getVFRB() { return $this->VFRB; }

	
	public function getBK() { return $this->BK; }

	
	public function getWVFRBO() { return $this->WVFRBO; }
	...
	public function getVFRBS2() { return $this->VFRBS2; }

	
	public function getSOLZS() { return $this->SOLZS; }

	
	public function getVFRBS1() { return $this->VFRBS1; }
	...
	public function getTBSVORV() { return $this->TBSVORV; }
	...
	public function getRVSATZAN() { return $this->RVSATZAN; }
	...
	public function getBBGKVPV() { return $this->BBGKVPV; }
	...
	public function getGFB() { return $this->GFB; }
	...
	public function getW3STKL5() { return $this->W3STKL5; }

	
	public function getBBGRV() { return $this->BBGRV; }
	...
	public function getW1STKL5() { return $this->W1STKL5; }
	...
	self::$ZAHL10000=new BigDecimal(10000);
	...
	protected function MPARA() {
		...
		if($this->KRV < 2) /** &lt; = < */{
			if($this->KRV == 0) {
				$this->BBGRV = new BigDecimal(74400);/** Neu 2016 */
			} else {
				$this->BBGRV = new BigDecimal(64800);/** Neu 2016 */
			}
			$this->RVSATZAN = BigDecimal::valueOf(0.0935);
			$this->TBSVORV = BigDecimal::valueOf(0.64);/** Neu 2016 */
		} else {/** Nichts zu tun */
		}
		$this->BBGKVPV = new BigDecimal(50850);/** Neu 2016 */
		$this->KVSATZAN = ($this->KVZ->divide(self::$ZAHL100))->add(BigDecimal::valueOf(0.07));
		$this->KVSATZAG = BigDecimal::valueOf(0.07);
		if($this->PVS == 1) {
			$this->PVSATZAN = BigDecimal::valueOf(0.01675);
			$this->PVSATZAG = BigDecimal::valueOf(0.00675);
		} else {
			$this->PVSATZAN =  BigDecimal::valueOf(0.01175);
			$this->PVSATZAG =  BigDecimal::valueOf(0.01175);
		}
		if($this->PVZ == 1) {
			$this->PVSATZAN = $this->PVSATZAN->add(BigDecimal::valueOf(0.0025));
		}/** Anfang Neu 2016 */
		$this->W1STKL5 = new BigDecimal(10070);
		$this->W2STKL5 = new BigDecimal(26832);
		$this->W3STKL5 = new BigDecimal(203557);
		$this->GFB = new BigDecimal(8652);
		$this->SOLZFREI = new BigDecimal(972);/** Ende Neu 2016 */
	}
	...
	protected function MBERECH() {
		...
		$this->VFRB = (($this->ANP->add($this->FVB->add($this->FVBZ)))->multiply(self::$ZAHL100))->setScale(0, BigDecimal::$ROUND_DOWN);/** Neu 2016 */
		$this->MLSTJAHR();/** Anfang Neu 2016 */
		$this->WVFRB = (($this->ZVE->subtract($this->GFB))->multiply(self::$ZAHL100))->setScale(0, BigDecimal::$ROUND_DOWN);
		if($this->WVFRB->compareTo(BigDecimal::$ZERO) == -1) /** WVFRB < 0 */{
			$this->WVFRB = BigDecimal::valueOf(0);
		}/** Ende Neu 2016 */
		$this->LSTJAHR = ($this->ST->multiply(BigDecimal::valueOf($this->f)))->setScale(0,BigDecimal::$ROUND_DOWN);
		$this->UPLSTLZZ();
		$this->UPVKVLZZ();
		if($this->ZKF->compareTo(BigDecimal::$ZERO) == 1) /** ZKF > 0 */{
	...
	protected function MZTABFB() {
		...
		if($this->STKL == 1) {
			...
			$this->KFB= ($this->ZKF->multiply (BigDecimal::valueOf (7248)))->setScale (0, BigDecimal::$ROUND_DOWN);/** Neu 2016 */
		} else {
			if($this->STKL == 2) {
				...
				if($this->STKL == 3) {
					...
					if($this->STKL == 4) {
						$this->SAP= BigDecimal::valueOf (36);
						$this->KFB= ($this->ZKF->multiply (BigDecimal::valueOf (3624)))->setScale (0, BigDecimal::$ROUND_DOWN);/** Neu 2016 */
					} else {
						if($this->STKL == 5) {
							$this->SAP= BigDecimal::valueOf (36);
	...
	protected function MST5_6() {
		...
		if($this->ZZX->compareTo($this->W2STKL5) == 1) {
			...
			if($this->ZZX->compareTo ($this->W1STKL5) == 1) {
				...
				$this->UP5_6();
				$this->HOCH= ($this->ST->add (($this->ZZX->subtract ($this->W1STKL5))->multiply (BigDecimal::valueOf (0.42))))->setScale (0, BigDecimal::$ROUND_DOWN);/** Neuer Wert 2014 */
	...
	protected function UP5_6() {
		...
		$this->X= ($this->ZX->multiply (BigDecimal::valueOf (1.25)))->setScale (2, BigDecimal::$ROUND_DOWN);/** Neu 2016 */
		$this->UPTAB16();
		$this->ST1= $this->ST;
		$this->X= ($this->ZX->multiply (BigDecimal::valueOf (0.75)))->setScale (2, BigDecimal::$ROUND_DOWN);/** Neu 2016 */
		$this->UPTAB16();
		$this->ST2= $this->ST;
	...
	protected function MSOLZ() {
		...
		$this->SOLZFREI = ($this->SOLZFREI->multiply(BigDecimal::valueOf($this->KZTAB)));/** Neu 2016 */
	...
	protected function MSONST() {
		...
		if($this->SONSTB->compareTo (BigDecimal::$ZERO) == 0) {
			...
			$this->MRE4SONST();
			$this->MLSTJAHR();/** Anfang Neu 2016 */
			$this->WVFRBM = ($this->ZVE->subtract($this->GFB))->multiply(self::$ZAHL100)->setScale(2,BigDecimal::$ROUND_DOWN);
			if($this->WVFRBM->compareTo(BigDecimal::$ZERO) == -1) /** WVFRBM < 0 */{
	...
	protected function MOSONST() {
		...
		$this->MZTABFB();/** Neu 2016 */
		$this->VFRBS1 = (($this->ANP->add($this->FVB->add($this->FVBZ)))->multiply(self::$ZAHL100))->setScale(2,BigDecimal::$ROUND_DOWN);
		$this->MLSTJAHR();/** Anfang Neu 2016 */
		$this->WVFRBO = (($this->ZVE->subtract($this->GFB))->multiply(self::$ZAHL100))->setScale(2, BigDecimal::$ROUND_DOWN);
	...
	protected function MRE4SONST() {
		...
		$this->MZTABFB();/** Neu 2016 */
		$this->VFRBS2 = (((($this->ANP->add($this->FVB)->add($this->FVBZ)))->multiply(self::$ZAHL100)))->subtract($this->VFRBS1);
	}

	/** Komplett Neu 2016 */
	/** Tarifliche Einkommensteuer §32a EStG, PAP Seite 36 */
	protected function UPTAB16() {

		if($this->X->compareTo($this->GFB->add(self::$ZAHL1)) == -1) {
			$this->ST= BigDecimal::$ZERO;
		} else {
			if($this->X->compareTo (BigDecimal::valueOf (13670)) == -1) {
				$this->Y = ($this->X->subtract($this->GFB))->divide(self::$ZAHL10000, 6,BigDecimal::$ROUND_DOWN);
				$this->RW= $this->Y->multiply (BigDecimal::valueOf (993.62));
				$this->RW= $this->RW->add (BigDecimal::valueOf (1400));
				$this->ST= ($this->RW->multiply ($this->Y))->setScale (0, BigDecimal::$ROUND_DOWN);
			} else {
				if($this->X->compareTo (BigDecimal::valueOf (53666)) == -1) {
					$this->Y= ($this->X->subtract (BigDecimal::valueOf (13669)))->divide (self::$ZAHL10000, 6, BigDecimal::$ROUND_DOWN);
					$this->RW= $this->Y->multiply (BigDecimal::valueOf (225.40));
					$this->RW= $this->RW->add (BigDecimal::valueOf (2397));
					$this->RW= $this->RW->multiply ($this->Y);
					$this->ST= ($this->RW->add (BigDecimal::valueOf (952.48)))->setScale (0, BigDecimal::$ROUND_DOWN);
				} else {
					if($this->X->compareTo (BigDecimal::valueOf (254447)) == -1) {
						$this->ST= (($this->X->multiply (BigDecimal::valueOf (0.42)))->subtract (BigDecimal::valueOf (8394.14)))->setScale (0, BigDecimal::$ROUND_DOWN);
					} else {
						$this->ST= (($this->X->multiply (BigDecimal::valueOf (0.45)))->subtract (BigDecimal::valueOf (16027.52)))->setScale (0, BigDecimal::$ROUND_DOWN);
					}
				}
			}
		}
