...
class Lohnsteuer2012 implements LohnsteuerInterface {
	...
	protected static final $RENTBEMESSUNGSGR_OST_2012 = new BigDecimal(57600);

	/** Rentenbemessungs-Grenze alte Bundesländer in EUR */
	protected static final $RENTBEMESSUNGSGR_WEST = new BigDecimal(67200);
	...
	protected function UPEVP() {
		...
		if($KRV > 1) {
			...
			if($KRV == 0) {
				if($ZRE4VP->compareTo(Lohnsteuer2012->self::$RENTBEMESSUNGSGR_WEST) == 1) {
					$ZRE4VP = Lohnsteuer2012->self::$RENTBEMESSUNGSGR_WEST;
				}
			} else {
				if($ZRE4VP->compareTo(Lohnsteuer2012->self::$RENTBEMESSUNGSGR_OST_2012) == 1) {
					$ZRE4VP = Lohnsteuer2012->self::$RENTBEMESSUNGSGR_OST_2012;
				}
			}
