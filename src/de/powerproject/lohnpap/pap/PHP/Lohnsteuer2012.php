...
/**
 * Klasse Lohnsteuer2012
 * 
 * @author Christian Delfs
 */

class Lohnsteuer2012 implements LohnsteuerInterface {
	...
	protected static $RENTBEMESSUNGSGR_OST_2012;
	...
	self::$RENTBEMESSUNGSGR_OST_2012=new BigDecimal(57600);
	...
	self::$RENTBEMESSUNGSGR_WEST=new BigDecimal(67200);
	...
	protected function UPEVP() {
		...
		if($this->KRV > 1) {
			...
			if($this->KRV == 0) {
				if($this->ZRE4VP->compareTo(Lohnsteuer2012->self::$RENTBEMESSUNGSGR_WEST) == 1) {
					$this->ZRE4VP = Lohnsteuer2012->self::$RENTBEMESSUNGSGR_WEST;
				}
			} else {
				if($this->ZRE4VP->compareTo(Lohnsteuer2012->self::$RENTBEMESSUNGSGR_OST_2012) == 1) {
					$this->ZRE4VP = Lohnsteuer2012->self::$RENTBEMESSUNGSGR_OST_2012;
				}
			}
			$this->VSP1 = $this->ZRE4VP->multiply(BigDecimal::valueOf(0.48))->setScale(2,BigDecimal::$ROUND_DOWN);
			$this->VSP1 = $this->VSP1->multiply(BigDecimal::valueOf(0.098))->setScale(2,BigDecimal::$ROUND_DOWN);
		}
	...
	protected function MVSP() {
		...
		if($this->ZRE4VP->compareTo( BigDecimal::valueOf(45900) ) == 1) {
			$this->ZRE4VP = BigDecimal::valueOf(45900);
		}
		if($this->PKV > 0) {
			if($this->STKL == 6) {
