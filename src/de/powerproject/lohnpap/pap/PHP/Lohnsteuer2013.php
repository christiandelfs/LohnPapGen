...
/**
 * Klasse Lohnsteuer2013
 * 
 * @author Christian Delfs
 */

class Lohnsteuer2013 implements LohnsteuerInterface {
	...
	/** Neuer Wert  2013 */
	protected static $RENTBEMESSUNGSGR_OST_2013;
	...
	self::$RENTBEMESSUNGSGR_OST_2013=new BigDecimal(58800);
	...
	self::$RENTBEMESSUNGSGR_WEST=new BigDecimal(69600);
	...
	protected function UPEVP() {
		...
		if($this->KRV > 1) {
			...
			if($this->KRV == 0) {/** Neuer Wert 2013 */
				if($this->ZRE4VP->compareTo(self::$RENTBEMESSUNGSGR_WEST) == 1) {
					$this->ZRE4VP = self::$RENTBEMESSUNGSGR_WEST;
				}
			} else {/** Neuer Wert 2013 */
				if($this->ZRE4VP->compareTo(self::$RENTBEMESSUNGSGR_OST_2013) == 1) {
					$this->ZRE4VP = self::$RENTBEMESSUNGSGR_OST_2013;
				}
			}/** Neuer Wert 2013 */
			$this->VSP1 = ($this->ZRE4VP->multiply(BigDecimal::valueOf(0.52)))->setScale(2,BigDecimal::$ROUND_DOWN);
...
/** Neuer Wert 2013 */
		if($this->ZRE4VP->compareTo( BigDecimal::valueOf(47250) ) == 1) {/** Neuer Wert 2013 */
			$this->ZRE4VP = BigDecimal::valueOf(47250);
		}
		if($this->PKV > 0) {
			if($this->STKL == 6) {
	...
	protected function MST5_6() {
		...
		if($this->ZZX->compareTo (BigDecimal::valueOf (26441)) == 1) {
			...
			$this->ZX= $this->ZZX;
			$this->UP5_6();
			if($this->ZZX->compareTo (BigDecimal::valueOf (9550)) == 1) {
				$this->VERGL= $this->ST;
				$this->ZX= BigDecimal::valueOf (9550);
				$this->UP5_6();
				$this->HOCH= ($this->ST->add (($this->ZZX->subtract (BigDecimal::valueOf (9550)))->multiply (BigDecimal::valueOf (0.42))))->setScale (0, BigDecimal::$ROUND_DOWN);
	...
	protected function UPTAB13() {

		if($this->X->compareTo (BigDecimal::valueOf (8131)) == -1) {
			$this->ST= BigDecimal::$ZERO;
		} else {
			if($this->X->compareTo (BigDecimal::valueOf (13470)) == -1) {
				$this->Y= ($this->X->subtract (BigDecimal::valueOf (8130)))->divide (BigDecimal::valueOf (10000), 6, BigDecimal::$ROUND_DOWN);
				$this->RW= $this->Y->multiply (BigDecimal::valueOf (933.7));
				...
				if($this->X->compareTo (BigDecimal::valueOf (52882)) == -1) {
					...
					$this->ST= ($this->RW->add (BigDecimal::valueOf (1014)))->setScale (0, BigDecimal::$ROUND_DOWN);
				} else {
					if($this->X->compareTo (BigDecimal::valueOf (250731)) == -1) {
						$this->ST= (($this->X->multiply (BigDecimal::valueOf (0.42)))->subtract (BigDecimal::valueOf (8196)))->setScale (0, BigDecimal::$ROUND_DOWN);
					} else {
						$this->ST= (($this->X->multiply (BigDecimal::valueOf (0.45)))->subtract (BigDecimal::valueOf (15718)))->setScale (0, BigDecimal::$ROUND_DOWN);
					}
				}
			}
		}
