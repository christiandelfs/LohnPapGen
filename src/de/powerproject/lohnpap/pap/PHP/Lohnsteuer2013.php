...
class Lohnsteuer2013 implements LohnsteuerInterface {
	...
	/** Neuer Wert  2013 */
	protected static final $RENTBEMESSUNGSGR_OST_2013 = new BigDecimal(58800);

	/** Rentenbemessungs-Grenze alte Bundesländer in EUR */

	/** Neuer Wert 2013 */
	protected static final $RENTBEMESSUNGSGR_WEST = new BigDecimal(69600);
	...
	protected function UPEVP() {
		...
		if($KRV > 1) {
			...
			if($KRV == 0) {/** Neuer Wert 2013 */
				if($ZRE4VP->compareTo(self::$RENTBEMESSUNGSGR_WEST) == 1) {
					$ZRE4VP = self::$RENTBEMESSUNGSGR_WEST;
				}
			} else {/** Neuer Wert 2013 */
				if($ZRE4VP->compareTo(self::$RENTBEMESSUNGSGR_OST_2013) == 1) {
					$ZRE4VP = self::$RENTBEMESSUNGSGR_OST_2013;
				}
			}/** Neuer Wert 2013 */
