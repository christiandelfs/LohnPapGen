...
class Lohnsteuer2007 implements LohnsteuerInterface {
	...
	protected function MRE4LZZ() {
		...
		if($VBEZ->compareTo (BigDecimal::$ZERO) == 0) {
			...
			if($LZZ == 1) {
				if((($STERBE->add ($VKAPA)).compareTo (BigDecimal::$ZERO)) == 1) {
					...
					$HFVB= self::$TAB2[$J].multiply (self::$ZAHL100);
					$FVBZ= self::$TAB3[$J];
				} else {
					$VBEZB= ($VBEZM->multiply (BigDecimal::valueOf ($ZMVB))).add ($VBEZS);/** Achtung! Rechengenauigkeit Division? */
					$HFVB= self::$TAB2[$J].divide (self::$ZAHL12).multiply (BigDecimal::valueOf ($ZMVB)).multiply (self::$ZAHL100);
			...
			$FVB= ($VBEZB->multiply (self::$TAB1[$J])).setScale (2, BigDecimal::$ROUND_UP);
		...
		if($ALTER1 == 0) {
			...
			$JW= self::$TAB5[$K].multiply (self::$ZAHL100);
			UPANTEIL();
...
/** Achtung: Es wird davon ausgegangen, dass<br>
	...
	protected function MSOLZ() {
		...
		if($JBMG->compareTo ($SOLZFREI) == 1) {
			...
			$SOLZMIN= ($JBMG->subtract ($SOLZFREI)).multiply (BigDecimal::valueOf (20)).divide (self::$ZAHL100);
