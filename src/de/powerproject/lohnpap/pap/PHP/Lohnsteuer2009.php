...
class Lohnsteuer2009 implements LohnsteuerInterface {
	...
	protected function MZTABFB() {
		...
		if($this->STKL == 1) {
			...
			$this->KFB= ($this->ZKF->multiply (BigDecimal::valueOf (6024)))->setScale (0, BigDecimal::$ROUND_DOWN);
		} else {
			if($this->STKL == 2) {
				...
				if($this->STKL == 3) {
					...
					if($this->STKL == 4) {
						$this->SAP= BigDecimal::valueOf (36);
						$this->KFB= ($this->ZKF->multiply (BigDecimal::valueOf (3012)))->setScale (0, BigDecimal::$ROUND_DOWN);
					} else {
...
/** Achtung: Er wird davon ausgegangen, dass<br>
    ...
    b) der Beitragsatz zur Rentenversicherung gegenüber 2008 unverändert bleibt */
		if($this->KRV > 0) {
			...
			if($this->ZRE4VP->compareTo (BigDecimal::valueOf (64800)) == 1) {
				$this->ZRE4VP= BigDecimal::valueOf (64800);
			}
			$this->VSP1= ($this->ZRE4VP->multiply (BigDecimal::valueOf (0.36)))->setScale (2, BigDecimal::$ROUND_DOWN);
	...
	protected function MST5_6() {
		...
		$this->ZZX= $this->X;
		if($this->ZZX->compareTo (BigDecimal::valueOf (26276)) == 1) {
			$this->ZX= BigDecimal::valueOf (26276);
			$this->UP5_6();
			if($this->ZZX->compareTo (BigDecimal::valueOf (200320)) == 1) {
				$this->ST= ($this->ST->add ((BigDecimal::valueOf (200320)->subtract (BigDecimal::valueOf (26276)))->multiply (BigDecimal::valueOf (0.42))))->setScale (0, BigDecimal::$ROUND_DOWN);
				$this->ST= ($this->ST->add (($this->ZZX->subtract (BigDecimal::valueOf (200320)))->multiply (BigDecimal::valueOf (0.45))))->setScale (0, BigDecimal::$ROUND_DOWN);
			} else {
				$this->ST= ($this->ST->add (($this->ZZX->subtract (BigDecimal::valueOf (26276)))->multiply (BigDecimal::valueOf (0.42))))->setScale (0, BigDecimal::$ROUND_DOWN);
			}
		} else {
			$this->ZX= $this->ZZX;
			$this->UP5_6();
			if($this->ZZX->compareTo (BigDecimal::valueOf (9225)) == 1) {
				$this->VERGL= $this->ST;
				$this->ZX= BigDecimal::valueOf (9225);
				$this->UP5_6();
				$this->HOCH= ($this->ST->add (($this->ZZX->subtract (BigDecimal::valueOf (9225)))->multiply (BigDecimal::valueOf (0.42))))->setScale (0, BigDecimal::$ROUND_DOWN);
	...
	protected function UPTAB09() {

		if($this->X->compareTo (BigDecimal::valueOf (7835)) == -1) {
			$this->ST= BigDecimal::$ZERO;
		} else {
			if($this->X->compareTo (BigDecimal::valueOf (13140)) == -1) {
				$this->Y= ($this->X->subtract (BigDecimal::valueOf (7834)))->divide (BigDecimal::valueOf (10000), 6, BigDecimal::$ROUND_DOWN);
				$this->RW= $this->Y->multiply (BigDecimal::valueOf (939.68));
				$this->RW= $this->RW->add (BigDecimal::valueOf (1400));
				$this->ST= ($this->RW->multiply ($this->Y))->setScale (0, BigDecimal::$ROUND_DOWN);
			} else {
				if($this->X->compareTo (BigDecimal::valueOf (52552)) == -1) {
					$this->Y= ($this->X->subtract (BigDecimal::valueOf (13139)))->divide (BigDecimal::valueOf (10000), 6, BigDecimal::$ROUND_DOWN);
					...
					$this->ST= ($this->RW->add (BigDecimal::valueOf (1007)))->setScale (0, BigDecimal::$ROUND_DOWN);
				} else {
					if($this->X->compareTo (BigDecimal::valueOf (250401)) == -1) {
						$this->ST= (($this->X->multiply (BigDecimal::valueOf (0.42)))->subtract (BigDecimal::valueOf (8064)))->setScale (0, BigDecimal::$ROUND_DOWN);
					} else {
						$this->ST= (($this->X->multiply (BigDecimal::valueOf (0.45)))->subtract (BigDecimal::valueOf (15576)))->setScale (0, BigDecimal::$ROUND_DOWN);
					}
				}
			}
		}
